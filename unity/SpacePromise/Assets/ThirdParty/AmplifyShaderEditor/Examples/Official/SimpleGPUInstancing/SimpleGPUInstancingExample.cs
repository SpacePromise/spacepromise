...
namespace Assets.ThirdParty.AmplifyShaderEditor.Examples.Official.SimpleGPUInstancing
{
    public class SimpleGPUInstancingExample : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            {
                Transform newInstance = Instantiate( this.Prefab, new Vector3( Random.Range( -range, range ), range + Random.Range( -range, range ), Random.Range( -range, range ) ), Quaternion.identity ) as Transform;
