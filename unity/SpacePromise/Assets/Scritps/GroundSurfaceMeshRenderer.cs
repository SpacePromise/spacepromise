...
public class GroundSurfaceMeshRenderer : MonoBehaviour
{
    ...
    private void RefreshLod()
    {
        ...
        {
            if (this.mesh != null)
                PlaneMeshPool.Instance.Return(this.mesh, this.lodSize, this.lodSize);

            // Calculate new terrain info
            this.currentLod = this.Lod;
            this.lodSize = LodBase / this.currentLod;
            this.renderScale = (float)LodBase / this.lodSize;

            // Generate new mesh
            this.mesh = PlaneMeshPool.Instance.Take(this.lodSize, this.lodSize);
